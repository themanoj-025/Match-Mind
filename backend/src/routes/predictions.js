...
router.post('/', authenticateToken, predictionLimiter, validate(createPredictionSchema), async (req, res, next) => {
  try {
    const prisma = req.app.get('prisma')
    const { matchId, homeGoals, awayGoals, firstScorer, totalGoalsOU, totalGoalsLine, btts } = req.body
    ...
    const prediction = await prisma.prediction.create({
      data: { userId: req.userId, matchId, homeGoals, awayGoals, firstScorer, totalGoalsOU, totalGoalsLine, btts, status: 'PENDING' },
    })
...
router.patch('/:id/score', validate(scorePredictionSchema), async (req, res, next) => {
  try {
    const prisma = req.app.get('prisma')

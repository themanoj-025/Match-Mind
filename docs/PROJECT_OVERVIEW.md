...
- **AI Draft Insights (Pro):** Subscribers get access to real-time AI heuristics that identify undervalued sleepers in the remaining player pool.
...
- **Execution order**:
  ...
  5. Apply middleware: helmet, cors, pino-http, Stripe webhook raw body, json, cookieParser, passport
  6. Mount 16 route groups under `/api/`
  7. Mount health check endpoint (with DB status check)
  8. Mount error handler
  9. Setup Socket.IO event handlers
  10. Initialize BullMQ workers with fallback
  11. Schedule weekly/monthly leaderboard resets (with safeSetTimeout to avoid 32-bit overflow)
  12. Start HTTP server
  13. Handle SIGTERM/SIGINT for graceful shutdown (with 10s timeout)
...
5. Opens browser at `http://localhost:3000`
   _(No external database server needed — JSON Database loads automatically)_
...
_This document describes the MatchMind repository based on source code analysis. Every statement is verified against the code. Where features are listed in documentation but not found in code, this is explicitly noted as "Not Implemented"._

_Generated: July 3, 2026 | Version: 1.0_

...
export default function MyProfilePage() {
  ...
  const stats = [
    ...
    {
      label: 'Points',
      value: user?.totalPoints ? user.totalPoints.toLocaleString() : '0',
  ...
  return (
    <motion.div className="min-h-screen pt-16 pb-20 md:pb-8">
      <Helmet>
        <title>My Profile — MatchMind</title>
      </Helmet>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="flex items-center justify-end gap-2 mb-4">
          <Link
            ...
            className="p-2 text-[var(--mm-text-secondary)] hover:text-[var(--mm-text-primary)] transition-colors"
          ...
          <div className="px-6 pb-6 -mt-12 sm:-mt-16">
            <div className="flex items-end gap-4 mb-4">
              ...
              <div className="pb-1 flex-1">
                ...
                <Link
                  to="/profile/me/settings"
                  className="inline-flex items-center gap-1 mt-2 caption text-[var(--mm-accent-green)] font-medium hover:underline"
                >
            ...
            <div className="grid grid-cols-4 gap-3">
              {stats.map((stat, i) => (
                <div key={i} className="bg-[var(--mm-bg-tertiary)] rounded-[var(--radius-md)] p-3 text-center">
                  <span className="block font-[var(--font-display)] text-xl" style={{ color: stat.color }}>
                    {stat.value}
                  </span>
              ...
              <div className="space-y-6">
                <div className="bg-[var(--mm-bg-secondary)] border border-[var(--border-subtle)] rounded-[var(--radius-xl)] p-5 sm:p-6">
                  ...
                  <div className="flex gap-3">
                    ...
                    <Link
                      to="/profile/me"
                      className="flex items-center text-[var(--mm-text-muted)] hover:text-[var(--mm-text-secondary)] transition-colors"
                    >
                ...
                <div className="bg-[var(--mm-bg-secondary)] border border-[var(--border-subtle)] rounded-[var(--radius-xl)] p-5 sm:p-6">
                  ...
                  <div className="text-center py-8 text-[var(--mm-text-muted)]">
                    ...
                    <button
                      ...
                      className="caption text-[var(--mm-accent-green)] font-medium mt-1 hover:underline"
                    >
                      Browse live matches
                    </button>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Link
                    to="/leagues"
                    className="bg-[var(--mm-bg-secondary)] border border-[var(--border-subtle)] rounded-[var(--radius-lg)] p-4 hover:border-[var(--border-active)] transition-all"
                  >
                    ...
                    recentPredictions.map((pred: any, i: number) => (
                      ...
                      >
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span
                            className={`px-2 py-0.5 rounded-[var(--radius-sm)] caption font-semibold ${
                              pred.status === 'CORRECT'
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <span className="body font-semibold text-[var(--mm-accent-amber)]">
              ...
              <div className="bg-[var(--mm-bg-secondary)] border border-[var(--border-subtle)] rounded-[var(--radius-xl)] p-5 sm:p-6">
                ...
                <div className="grid grid-cols-3 sm:grid-cols-5 gap-4">
                  {achievements.map((badge, i) => (
                    <div
                      key={i}
                      className={`rounded-[var(--radius-lg)] p-3 text-center transition-all ${
                        badge.unlocked
                          ? 'bg-[var(--mm-bg-tertiary)] border border-[var(--border-active)]/30'
                          : 'bg-[var(--mm-bg-tertiary)]/50 border border-[var(--border-subtle)]'
                      }`}
                    >
                      <span className={`text-3xl block mb-1 ${badge.unlocked ? '' : 'opacity-30 grayscale'}`}>
                        {badge.unlocked ? badge.icon : '🔒'}
                      </span>
                      <span
                        className={`caption font-medium ${badge.unlocked ? 'text-[var(--mm-text-secondary)]' : 'text-[var(--mm-text-muted)]'}`}
                      >
                        {badge.name}
                      </span>
                      <span
                        className={`caption block mt-0.5 ${badge.unlocked ? 'text-[var(--mm-text-muted)]' : 'text-[var(--mm-text-muted)]'}`}
                      >

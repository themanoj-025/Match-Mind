...
export default function ProfilePage() {
  ...
  return (
    <motion.div className="min-h-screen pt-16 pb-20 md:pb-8">
      <Helmet>
        <title>{user.name} (@{user.username}) — Profile & Stats | MatchMind</title>
        <meta name="description" content={`${user.name} — ${user.accuracy}% accuracy, ${user.points.toLocaleString()} pts, #${user.rank} Global. View predictions, achievements, and stats.`} />
        <meta property="og:title" content={`${user.name} (@${user.username}) — MatchMind Profile`} />
        <meta property="og:description" content={`🎯 ${user.accuracy}% Accuracy • 🪙 ${user.points.toLocaleString()} pts • 🏆 #${user.rank} Global • 🔥 ${user.streak}-day streak`} />
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="bg-[var(--mm-bg-secondary)] border border-[var(--border-subtle)] rounded-[var(--radius-xl)] overflow-hidden mb-6">
          <div className="h-32 sm:h-48 bg-gradient-to-r from-[var(--mm-accent-green)]/20 via-[var(--mm-accent-blue)]/20 to-[var(--mm-accent-purple)]/20 relative">
            <div className="absolute inset-0 opacity-10" style={{ backgroundImage: 'url("data:image/svg+xml,%3Csvg width=\'60\' height=\'60\' viewBox=\'0 0 60 60\' xmlns=\'http://www.w3.org/2000/svg\'%3E%3Cg fill=\'none\' fill-rule=\'evenodd\'%3E%3Cg fill=\'%2300E676\' fill-opacity=\'0.1\'%3E%3Cpath d=\'M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z\'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E")' }} />
          </div>
          <div className="px-6 pb-6 -mt-12 sm:-mt-16">
            <div className="flex items-end gap-4 mb-4">
              ...
              <div className="pb-1 flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="px-2 py-0.5 bg-gradient-to-r from-[var(--mm-accent-purple)] to-[var(--mm-accent-blue)] rounded-[var(--radius-sm)] text-[10px] font-bold text-white">PRO</span>
                </div>
                ...
                <div className="flex items-center gap-3 mt-1.5">
                  <span className="caption text-[var(--mm-text-muted)] flex items-center gap-1"><MapPin size={12} /> {user.country}</span>
                  <span className="caption text-[var(--mm-text-muted)] flex items-center gap-1"><Calendar size={12} /> Joined {user.joined}</span>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <button
                ...
                <button className="p-2.5 text-[var(--mm-text-secondary)] hover:text-[var(--mm-text-primary)] transition-colors" aria-label="More options">
            ...
            <div className="grid grid-cols-4 gap-3">
              <div className="bg-[var(--mm-bg-tertiary)] rounded-[var(--radius-md)] p-3 text-center">
                <span className="block font-[var(--font-display)] text-xl text-[var(--mm-accent-green)]">{user.accuracy}%</span>
                <span className="caption text-[var(--mm-text-muted)]">Accuracy</span>
              </div>
              <div className="bg-[var(--mm-bg-tertiary)] rounded-[var(--radius-md)] p-3 text-center">
                <span className="block font-[var(--font-display)] text-xl text-[var(--mm-accent-amber)]">🪙{user.points.toLocaleString()}</span>
                <span className="caption text-[var(--mm-text-muted)]">Points</span>
              </div>
              <div className="bg-[var(--mm-bg-tertiary)] rounded-[var(--radius-md)] p-3 text-center">
                <span className="block font-[var(--font-display)] text-xl text-[var(--mm-accent-blue)]">#{user.rank}</span>
                <span className="caption text-[var(--mm-text-muted)]">Global Rank</span>
              </div>
              <div className="bg-[var(--mm-bg-tertiary)] rounded-[var(--radius-md)] p-3 text-center">
                <span className="block font-[var(--font-display)] text-xl text-[var(--mm-accent-red)]">🔥{user.streak}</span>
            ...
            <div className="flex items-center gap-4 mt-3">
              <div className="flex items-center gap-1.5">
                ...
                {user.sports.map((s, i) => <span key={i} className="text-lg">{s}</span>)}
              </div>
              <div className="flex items-center gap-1.5">
                <span className="caption text-[var(--mm-text-muted)]">Teams:</span>
                {user.teams.map((t, i) => <span key={i} className="caption text-[var(--mm-text-secondary)]">{t}{i < user.teams.length - 1 ? ',' : ''}</span>)}
              </div>
            </div>
          </div>
        </div>
                  ...
                  {[
                    ...
                    <div key={i} className="flex items-start gap-3 py-2 border-b border-[var(--border-subtle)] last:border-0">
                      <div className="w-8 h-8 rounded-full bg-[var(--mm-bg-tertiary)] flex items-center justify-center shrink-0 text-sm">
                        {item.type === 'prediction' ? '🎯' : item.type === 'points' ? '🪙' : item.type === 'rank' ? '🏆' : item.type === 'achievement' ? '🏅' : '👥'}
                      </div>
                      <div>

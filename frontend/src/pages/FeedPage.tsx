...
export default function FeedPage() {
  ...
  const liveMatches: Match[] = allMatches.filter((m: Match) => m.status === 'SIMULATING')
  const upcomingMatches: Match[] = allMatches.filter((m: Match) => m.status === 'SCHEDULED')

  const topPredictors = topUsers.slice(0, 3).map((u: LeaderboardEntry, i: number) => ({

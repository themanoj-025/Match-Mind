...
const faqs = [
  {
    ...
    a: 'Yes, cancel anytime from Settings → Billing. You keep Pro benefits until the end of your billing period.',
  },
  { q: 'Is there a free trial?', a: 'We offer a 14-day no-questions-asked refund policy. Try Pro risk-free.' },
  {
    q: 'What payment methods are accepted?',
    a: 'We accept all major credit cards via Stripe. Apple Pay is available on iOS.',
  },
  ...
  return (
    <motion.div className="min-h-screen pt-16 pb-20">
      <Helmet>
        ...
        <meta
          name="description"
          content="MatchMind Pro — $4.99/month or $39.99/year. Unlock AI predictions, advanced analytics, exclusive badges, and more."
        />
      </Helmet>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="flex items-center justify-center gap-3 mb-10">
          <span
            className={`body font-medium ${!isAnnual ? 'text-[var(--mm-text-primary)]' : 'text-[var(--mm-text-muted)]'}`}
          >
            Monthly
          </span>
          <button
          ...
          >
            <span
              className={`absolute top-1 w-5 h-5 rounded-full bg-white transition-transform duration-300 ${isAnnual ? 'translate-x-8' : 'translate-x-1'}`}
            />
          </button>
          <span
            className={`body font-medium ${isAnnual ? 'text-[var(--mm-text-primary)]' : 'text-[var(--mm-text-muted)]'}`}
          ...
          {plans.map((plan, i) => (
            ...
            >
              ...
              <div className="mb-6">
                ...
                {plan.period && (
                  <span className="body text-[var(--mm-text-muted)] ml-1">
                    {isAnnual ? plan.period.annual : plan.period.monthly}
                  </span>
                )}
                {isAnnual && plan.highlighted && (
                  <span className="block caption text-[var(--mm-accent-green)] font-medium mt-1">
                    $39.99/year — Save 33%
                  </span>
                )}
              </div>
        ...
        <div className="max-w-2xl mx-auto mt-16">
          ...
          <div className="space-y-3">
            {faqs.map((faq, i) => (
              <details
                key={i}
                className="bg-[var(--mm-bg-secondary)] border border-[var(--border-subtle)] rounded-[var(--radius-lg)] group"
              >
                <summary className="px-5 py-4 body font-medium cursor-pointer list-none flex items-center justify-between group-open:text-[var(--mm-accent-green)]">
                  {faq.q}
                  <span className="text-[var(--mm-text-muted)] group-open:rotate-180 transition-transform duration-200">
                    ▼
                  </span>
                </summary>

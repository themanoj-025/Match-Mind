...
export default function ChatMessage({ message, isOwn, onReact, onReport, onPin }: ChatMessageProps) {
  ...
  return (
    <div
      className={`flex gap-2.5 px-4 py-2 hover:bg-[var(--mm-bg-hover)]/30 transition-colors group ${isPinned ? 'bg-[var(--mm-accent-amber)]/5 border-l-2 border-[var(--mm-accent-amber)]' : ''}`}
    >
      {/* Avatar */}
        ...
        <div className="flex items-center gap-1.5 mt-1.5">
          ...
          <div className="ml-auto opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1">
            {onPin && (
              <button
                onClick={() => onPin(id)}
                className="p-1 rounded-[var(--radius-sm)] hover:bg-[var(--mm-bg-hover)] text-[var(--mm-text-muted)] hover:text-[var(--mm-text-secondary)]"
